...
- Ordinary `figma:inspect` is a single-call paginated live read. It accepts `--depth 0`, optional `--fields`, and an opaque `--cursor`; each page reports flat `nodes`, `hasMore`, and `nextCursor`. The cursor is bound to the file, node, depth, fields, and offset. Pages are not a snapshot, so callers must tolerate duplicates or omissions when the tree changes between calls.
...
- When a command needs a persisted result (for omitted inline output, a remote error, or unrendered non-text content), the runtime publishes one complete `outputFiles.resultFile` receipt (`kind: "figma-cli-result"`, `schemaVersion: 1`) with `jq.full`, `jq.status`, and command-specific `jq.data` filters. Direct and typed protocol data is sanitized into optional `upstream`; `figma:run` keeps normalized execution data in `result`. Protocol `_meta` and tool-definition annotations are removed while business `_meta` inside `structuredContent` remains unchanged. An over-budget direct response returns a bounded resource-limit diagnostic without persisting its payload. Local artifacts, captures, downloads, receipts, and same-machine locks are invocation-local operational data. Keep them out of Git and preserve the runtime's managed-path and atomic-write boundaries. The fileKey lock only serializes `figma:run`, `figma:assets:apply`, `figma:code-connect:apply`, and `figma:upstream:call` when it resolves a fileKey.
...
- Limits, target schema, OAuth behavior, result receipt format, and JSON input shape belong to code, generated help, and contract tests. Do not duplicate their exact values here.
...
5. For filesystem, result receipt, capture, download, or OAuth changes, reuse the shared primitives and retain known operation outcomes through local failures. Test retries, concurrent access, partial-output cleanup, and terminal versus transient authentication behavior at the owning layer.
