...
- `figma_workspace_call_upstream_tool` is only for explicit uncovered upstream capabilities, including official shader effect/fill reads and official `export_video`; prefer first-class wrappers for metadata, screenshot/asset/download workflows, design context, motion context, and design-system reads.
