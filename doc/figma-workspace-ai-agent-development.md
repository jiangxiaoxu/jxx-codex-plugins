...
- State, workspace, result, capture, and OAuth data are local operational state. Keep them out of Git and preserve the runtime's managed-path, atomic-write, and same-machine locking boundaries.
...
- Limits, state schema, OAuth behavior, sidecar format, and JSON input shape belong to code, generated help, and contract tests. Do not duplicate their exact values here.
