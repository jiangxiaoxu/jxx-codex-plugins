...
- `figma:run` reports `executionOutcome`: `not_started`, `failed_atomic`, `succeeded`, or `outcome_unknown`. A directly returned `use_figma` script error is `failed_atomic`: Figma confirmed no file changes, so repair and retry safely. Only an execution whose completion cannot be confirmed is `outcome_unknown` and requires read-back/reconciliation before retry. A confirmed operation followed by local post-processing failure remains `succeeded` and must not be replayed blindly.
...
- Local artifacts, capture, download, temporary sidecars, and same-machine locks are invocation-local operational data. Keep them out of Git and preserve the runtime's managed-path and atomic-write boundaries. The fileKey lock only serializes `figma:run`, `figma:assets:apply`, and `figma:upstream:call` when it resolves a fileKey.
...
The committed upstream contract fixture is the last reviewed baseline, not runtime truth. Promotion atomically replaces only that local fixture with a reviewed candidate, so its content and SHA-256 change by design. It does not modify the remote MCP, the GitHub upstream guide archive, canonical corpus, or agent-facing help and skill. Runtime wrappers continue to discover the live official schema and fail or filter according to their typed contract.
