...
- `figma_workspace_call_upstream_tool` is only for explicit uncovered upstream capabilities; prefer first-class wrappers for metadata, screenshot/asset/download workflows, design context, motion context, video export, design-system reads, and shader effect/fill reads.
