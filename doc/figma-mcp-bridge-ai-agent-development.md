...
- public tool schemas and docs expose the fixed response shape only. Upstream-backed execution/debug tools may expose compact upstream envelopes inline, fixed official wrappers keep generic upstream details out of normal inline results, result JSON files are minimal envelopes without full `session`, `outputFiles`, or paired sidecar paths, eval/script/upstream-tool result files never contain `upstream`, and `figma_repl_run_task_plan` omits the input-only `stopOnFailure` from inline/result-file output;
