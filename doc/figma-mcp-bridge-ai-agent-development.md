...
- every public `figma_repl_*` tool uses a fixed structured response shape: public session without `history`, diagnostics arrays present, file pointers under `outputFiles` as `{ path, bytes, lineCount }` when a tool writes file-pointer collections, upstream-backed single-call tools expose parsed JSON in `upstream.payload` or non-JSON output in `upstream.text`, and fixed official wrappers keep inline results business-focused with failures in `upstreamError` while full upstream envelopes/arguments stay only in explicit result files;
...
- public tool schemas and docs expose the fixed response shape only. Upstream-backed execution/debug tools may expose compact upstream envelopes inline, fixed official wrappers keep generic upstream details out of normal inline results, `figma_repl_run_task_plan` omits the input-only `stopOnFailure` from inline/result-file output, and `figma_repl_run_script_file` result files use the `upstream` envelope instead of duplicate `raw`;
