...
- every public `figma_repl_*` tool uses a fixed structured response shape: public session without `history`, diagnostics arrays present, file pointers under `outputFiles` as `{ path, bytes, lineCount }`, upstream-backed single-call tools expose parsed JSON in `upstream.payload` or non-JSON output in `upstream.text`, and asset manifests keep compact inline assets with full per-asset upstream details only in explicit result files;
