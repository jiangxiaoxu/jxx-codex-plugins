...
- `plugins/figma-workspace/skills/figma-workspace/references/canonical-corpus/` is the only bundled workflow corpus read at runtime. It is a self-contained, plugin-owned 87-record corpus built from local mirrors and policy: 46 `active`, 20 `conditional`, 12 `router`, and 9 `examples` records. The examples are plugin-owned TypeScript-in-Markdown templates. Upstream source text is not packaged or read at runtime.
...
- `figma:api:search` reads a plugin-owned symbol index generated during the package build from bundled `@figma/plugin-typings`; it does not read the development snapshot. `figma:doctor` diagnoses the canonical corpus, generated API index, project docs, and TypeScript runtime assets. Upstream drift belongs to the maintenance updater, not `doctor`.
