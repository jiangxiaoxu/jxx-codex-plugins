...
description: Inspect, set, or reset the current thread's rollover reminder policy when explicitly requested by the user or parent agent.
---
...
Only run `set` or `reset` for an explicit change request. Inspection or explanation does not authorize changes. With no action specified, `show` the policy and ask the requester what to do. High context usage, hook reminders, and ordinary task delegation are not reasons to activate this skill or change policy.
...
When presenting settings or choices, distinguish the override from the model default. Match the runtime- or user-supplied model slug exactly; if unknown, show both rows. The script does not detect the active model.
