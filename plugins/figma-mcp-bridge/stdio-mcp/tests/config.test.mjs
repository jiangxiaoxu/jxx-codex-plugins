...
test("createConfig applies Figma MCP defaults", () => {
  ...
  assert.equal(config.endpoint, DEFAULT_FIGMA_MCP_ENDPOINT);
  ...
  assert.match(config.statePath, /figma-mcp-bridge[\\/]stdio-mcp[\\/]\.mcp-oauth-state\.json$/);
...
test("createConfig uses USERPROFILE .codex as the bridge OAuth cache path", async () => {
  const dir = await mkdtemp(join(tmpdir(), "figma-stdio-home-config-"));
...
test("createConfig accepts an explicit shared OAuth cache path", () => {
  const config = createConfig({
    statePath: "C:/Users/jxx73/.codex/figma-mcp-oauth.json",
    openBrowser: false,
  });

  assert.match(config.statePath, /figma-mcp-oauth\.json$/);
});

test("createConfig uses FIGMA_MCP_OAUTH_CACHE_PATH first", async () => {
  const dir = await mkdtemp(join(tmpdir(), "figma-stdio-cache-path-"));
  try {
...
test("createConfig uses CODEX_HOME as the bridge OAuth cache location", async () => {
  const dir = await mkdtemp(join(tmpdir(), "figma-stdio-codex-home-"));
  try {
...
test("findCodexHomeOAuthCachePath uses USERPROFILE .codex when CODEX_HOME is missing", async () => {
  const dir = await mkdtemp(join(tmpdir(), "figma-stdio-home-"));
  try {
    const cachePath = join(dir, ".codex", ".figma-mcp-bridge-oauth.json");
