...
test("figma REPL exposes self-explaining capabilities and resources", async () => {
  ...
  assert.match(captureNodeTool.description, /Recommended call: \{ title, sessionId, target, outputFile \}/);
  ...
  assert.ok(callUpstreamTool.outputSchema.properties.outputFiles);
  assert.ok(callUpstreamTool.outputSchema.properties.inlineResultLimit);
  await assert.rejects(
    () => mcpClient.callTool({
...
test("figma REPL runtime parsers reject malformed tool argument shapes", async () => {
  ...
  await assert.rejects(
    ...
    /Tool argument "argumentsTemplate" was removed\. Use "arguments"\./,
  );
  await assert.rejects(
    mcpClient.callTool({
  ...
  try {
    ...
    const result = await mcpClient.callTool({
      ...
      arguments: {
        ...
        toolName: "fake_upload_asset",
        arguments: { file: "{{path}}", target: "{{target}}" },
      },
    });
    const json = structuredToolResult(result);
    assert.equal(json.ok, true);
  ...
  try {
    ...
    await mcpClient.callTool({
      ...
      arguments: {
        ...
        connect: false,
        file: "https://www.figma.com/design/EctrdKKdR3c8JTPl55qn3r/Untitled",
        handles: { "$hero": "22:9" },
      },
    });
    const result = await mcpClient.callTool({
        ...
        steps: [
          ...
          },
          {
            ...
            args: {
              ...
              toolName: "fake_asset",
              arguments: { path: "{{path}}", nodeId: "{{target}}" },
            },
          },
          {
            id: "capture",
...
test("figma REPL programmatic client returns typed output contracts", async () => {
  ...
  try {
    ...
    const assetResult = await repl.applyAssetManifest({
      ...
      toolName: "fake_asset",
      arguments: { file: "{{path}}", nodeId: "{{target}}" },
