...
export const FIGMA_REPL_HELPER_HARD_RULES = [
...
export const FIGMA_REPL_HELPER_CATEGORIES: FigmaReplHelperCategory[] = [
...
export const FIGMA_REPL_HELPER_PROFILES: FigmaReplHelperProfile[] = [
  {
    id: "selection",
    category: "selection",
...
export function chooseHelperProfilesForIntent(
  ...
  const tokens = tokenizeCatalogQuery(query);
  const lowerQuery = query.toLowerCase();
  const ranked = FIGMA_REPL_HELPER_PROFILES
    .map((profile) => ({
      profile,
...
function scoreHelperProfile(
  profile: FigmaReplHelperProfile,
  tokens: string[],
  lowerQuery: string,
): number {
