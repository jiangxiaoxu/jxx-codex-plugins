...
import type {
  ...
  FigmaReplSearchDesignSystemArguments,
...
import {
  ...
  isMissingFileError as isFigmaReplMissingFileErrorForTesting,
  loadTaskPlan,
 ...
 */
export { isFigmaReplMissingFileErrorForTesting };
...
const DEFAULT_EVAL_DESCRIPTION = "Figma REPL JavaScript execution";
...
export interface FigmaReplMetadataTreeNode {
  ...
  width?: number;
  height?: number;
  children?: FigmaReplMetadataTreeNode[];
}

export interface FigmaReplMetadataJson {
  [key: string]: unknown;
  ...
  nodeId?: string;
  nodeCount: number;
  root?: FigmaReplMetadataTreeNode;
}

export interface FigmaReplGetMetadataResult extends FigmaReplToolResultBase {
  session: FigmaReplCompactSession;
  fileKey: string;
  nodeId?: string;
  metadata: {
...
function resolveEvalOutputFile(session: FigmaReplSession): string {
...
async function writeMetadataFile(options: {
  args: FigmaReplGetMetadataArguments;
  session: FigmaReplSession;
  metadata: FigmaReplMetadataJson;
}): Promise<FigmaReplFilePointer> {
...
function metadataResultFilePath(session: FigmaReplSession): string {
...
function resolveCallUpstreamOutputFile(toolName: string, session: FigmaReplSession): string {
  const timestamp = new Date().toISOString().replace(/[^\dTZ]/gu, "");
...
async function executeSearchDesignSystem(
  ...
  const session = prepareFileScopedSession(args, runtime.sessions);
  const fileKey = resolveRequiredFileKey(args, session, "figma_repl_search_design_system");
  const query = args.query.trim();
  return executeDedicatedUpstreamTool({
    args,
    runtime,
    session,
    wrapperToolName: "figma_repl_search_design_system",
...
async function handleGetLibraries(
  args: FigmaReplGetLibrariesArguments,
  runtime: {
    client: FigmaMcpProxyClient;
    sessions: FigmaReplSessionStore;
    upstreamToolCache: ReturnType<typeof createUpstreamToolCache>;
...
async function executeGetLibraries(
  ...
  const session = prepareFileScopedSession(args, runtime.sessions);
  const fileKey = resolveRequiredFileKey(args, session, "figma_repl_get_libraries");
  return executeDedicatedUpstreamTool({
    args,
    runtime,
    session,
    wrapperToolName: "figma_repl_get_libraries",
...
async function executeGetVariableDefs(
  ...
  return executeDedicatedUpstreamTool({
    ...
    runtime,
    session,
    wrapperToolName: "figma_repl_get_variable_defs",
...
async function executeCallUpstreamTool(
  ...
  const outputFiles = await writeCallUpstreamResultFiles({
    toolName: args.toolName,
    wrapperToolName: "figma_repl_call_upstream_tool",
    session,
    resultPayload,
    upstream: upstreamEnvelope(parsed),
  });
...
function metadataTreeFromXmlElement(element: ParsedMetadataXmlElement): FigmaReplMetadataTreeNode {
  return removeUndefined({
...
function countMetadataTreeNodes(node: FigmaReplMetadataTreeNode): number {
...
function createTaskScriptTemplate(taskName: string, args: FigmaReplPrepareTaskArguments): string {
  return [
    `// ${taskName}.figma.js`,
...
function createCompactWrapperProfilePayloads(): Array<Record<string, unknown>> {
  return FIGMA_REPL_WRAPPER_LOOKUP_PROFILES.map((profile) => ({
    tool: profile.tool,
    upstreamTool: profile.upstreamTool,
    workflowIds: profile.workflowIds,
  }));
...
function createPublicWrapperWorkflowPayloads(
  workflows: FigmaReplWrapperWorkflow[],
): Array<Record<string, unknown>> {
...
function createCompactWrapperWorkflowPayloads(): Array<Record<string, unknown>> {
  return FIGMA_REPL_WRAPPER_WORKFLOW_GRAPH.map((workflow) => ({
    id: workflow.id,
    tools: workflow.tools,
  }));
}
...
function responseResourceWorkspace(workspace: FigmaReplSessionWorkspace): FigmaReplResourceWorkspace {
  return removeUndefined({
    sessionDir: workspace.sessionDir,
  }) as FigmaReplResourceWorkspace;
}

function responseResourcePage(session: FigmaReplSession): FigmaReplResourcePageState | undefined {
  ...
  }) as FigmaReplResourcePageState;
}

function sessionResourceSummary(session: FigmaReplSession): FigmaReplResourceSessionSummary {
  return removeUndefined({
    id: session.id,
    fileKey: session.fileKey,
    surface: session.surface,
    sessionDir: session.workspace?.sessionDir,
  }) as FigmaReplResourceSessionSummary;
}

function sessionResourceDetail(session: FigmaReplSession): FigmaReplResourceSessionDetail {
  return removeUndefined({
    id: session.id,
    fileKey: session.fileKey,
    surface: session.surface,
  ...
  }) as FigmaReplResourceSessionDetail;
}
