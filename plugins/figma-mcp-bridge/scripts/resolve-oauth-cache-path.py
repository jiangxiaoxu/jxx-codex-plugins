...
OAUTH_CACHE_FILENAME = ".figma-mcp-bridge-oauth.json"
...
def resolve_oauth_cache_path(env: Mapping[str, str]) -> OAuthCachePath | None:
    explicit = env.get("FIGMA_MCP_OAUTH_CACHE_PATH")
    if explicit:
        return OAuthCachePath(explicit, "FIGMA_MCP_OAUTH_CACHE_PATH")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if resolved is None:
        print(
            ...
            "Set FIGMA_MCP_OAUTH_CACHE_PATH, CODEX_HOME, or USERPROFILE.",
            file=sys.stderr,
        )
        return 1
