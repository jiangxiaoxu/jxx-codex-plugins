...
function Resolve-OAuthCachePath {
  ...
  if ($env:CODEX_HOME) {
    return (Join-Path $env:CODEX_HOME ".figma-workspace-oauth.json")
  }

  if ($env:USERPROFILE) {
    return (Join-Path (Join-Path $env:USERPROFILE ".codex") ".figma-workspace-oauth.json")
  }
...
if ($env:FIGMA_WORKSPACE_BRIDGE_LOGIN_CHILD -ne "1") {
  ...
  $launchCommand = '$env:FIGMA_WORKSPACE_BRIDGE_LOGIN_CHILD = "1"; & ''' + $escapedPath + ''''
