...
- For implementation context or motion, call `figma_workspace_get_design_context({ sessionId, target })` and `figma_workspace_get_motion_context({ sessionId, target, recursive: true })`; use `figma_workspace_export_video` only when official video frame sampling is worth the render cost.
- For visual QA, call `figma_workspace_capture_node({ sessionId, target, imageFile })` and inspect the local image file.
