...
- Direct upstream calls use the selected live schema and do not receive `.figma.ts` preflight, private PluginData checks, or typed argument validation. Their sanitized sidecar omits protocol `_meta` and tool annotations; it preserves business data, including a business `_meta` nested in `structuredContent`.
