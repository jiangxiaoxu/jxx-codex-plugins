...
- A direct `use_figma` call does not use `figma:run` TypeScript preflight, private PluginData checks, typed argument validation, or typed recovery presentation. Use it only when the live schema is needed, and preserve its sanitized result sidecar.
