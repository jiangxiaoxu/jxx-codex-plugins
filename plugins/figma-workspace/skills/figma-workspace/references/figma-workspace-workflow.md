...
- Use `figma_workspace_capture_node` for final visual QA captures saved as local PNG files. `maxDimension` and `contentsOnly` are the exposed screenshot tuning options; use the upstream escape hatch for other screenshot parameters. Raw node id or `$handle` string targets require session file context; node URL targets or `target:{ fileKey, nodeId }` can supply file context directly.
