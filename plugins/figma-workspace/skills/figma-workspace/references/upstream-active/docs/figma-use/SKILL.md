...
2.  **Write plain JavaScript with top-level `await` and `return`.** Write an ordinary async TypeScript body in a local `.figma.ts` file. Execute it explicitly with `figma:script:run`; the CLI does not execute pasted JavaScript automatically.
