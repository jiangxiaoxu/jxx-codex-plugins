...
- Use `figma:doctor` only for installed runtime, raw snapshot, derived active-index, pending-record, or TypeScript asset faults. OAuth errors follow the approval-gated login flow below.
...
4. Call `figma:guidance -- <keywords> --state-file <absolute-path>` with compact task keywords before non-trivial work. It is fixed to active upstream guidance. Use `figma:docs:search` for project/upstream workflow snippets and `figma:api:search` for exact Plugin API symbols, always with the same state file.
...
- Use `figma:api:search -- <symbol> --state-file <absolute-path>` for exact Plugin API details and the single API record. Use `figma:docs:search -- <query> --scope <active|conditional|examples|all> --state-file <absolute-path>` for workflow documentation; default `active` searches 46 active records plus project docs, `conditional` selects 20 records explicitly, `examples` selects 9 non-executable examples, and `all` additionally includes 12 router records. Snippets are capped rather than full declaration files.
- `figma:guidance` is fixed to the active scope. Project Markdown is indexed with active lookup; raw upstream snapshot data is not default retrieval.
- The bundled 88-record raw upstream snapshot provides pin and provenance data. Its derived active index is the lookup surface. Both JSONL corpora are internal data; do not read or route agents to them directly. Only the GitHub-only `npm run update:upstream-corpus -- --ref <git-ref>` updater regenerates the snapshot and publishes the derived index.
