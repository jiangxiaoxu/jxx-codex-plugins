...
- Use `figma:doctor` only for installed canonical corpus, generated Plugin API index, project-doc, or TypeScript asset faults. Upstream drift is a maintainer concern, and OAuth errors follow the approval-gated login flow below.
...
4. Call `figma:guidance -- <keywords> --state-file <absolute-path>` with compact task keywords before non-trivial work. It uses the `active` canonical scope. Use `figma:docs:search` for project and canonical workflow snippets and `figma:api:search` for exact Plugin API symbols, always with the same state file.
...
- Use `figma:api:search -- <symbol> --state-file <absolute-path>` for exact Plugin API details from the generated, bundled `@figma/plugin-typings` symbol index. Use `figma:docs:search -- <query> --scope <active|conditional|examples|all> --state-file <absolute-path>` for workflow documentation; default `active` searches 46 canonical records plus project docs, `conditional` selects 20 records explicitly, `examples` selects 9 plugin-owned TypeScript-in-Markdown templates, and `all` additionally includes 12 router records. Snippets are capped rather than full declaration files.
- `figma:guidance` uses the `active` canonical scope. Runtime workflow lookup reads only the plugin-owned 87-record `canonical-corpus`; it never reads upstream source text or the development snapshot. Internal corpus and API index files are not agent-facing documents.
