...
3. Read stdout as Restricted Markdown; never parse it as JSON. Keep the default inline-result threshold for normal agent calls. Local artifact reading and result-file handling are defined below; do not raise the inline threshold merely to avoid a result file.
...
- File-scoped work needs an explicit Figma file target. Node-scoped work needs a full node URL or an explicit file-plus-node target accepted by the selected leaf help.
- A URL `node-id` normalizes to the Plugin API node ID; the slug and tracking query parameters are not identity. A bare node ID is never sufficient.
- A Figma URL determines the Design, FigJam, or Slides surface. With a raw fileKey, provide a surface whenever the selected command requires one.
...
- Use the public Plugin API search/read commands for native declarations. The output is human-readable; when a bare selector is ambiguous, pass a qualified selector from search to read. Overloads for the selected owner are read together. API lookup does not require or expose declaration-file paths, source line numbers, or opaque IDs.
...
- Read the selected leaf help for numeric ranges and strict usage boundaries.
...
| --- | --- | --- |
| Find workflow docs | `figma:docs:help`, `figma:docs:list`, `figma:docs:catalog`, `figma:docs:search`, `figma:docs:read` | Targetless. |
| Find Plugin API | `figma:api:help`, `figma:api:search`, `figma:api:read` | Targetless. |
| Diagnose installed runtime assets | `figma:doctor` | Targetless. |
| Understand a Design file | `figma:metadata`, `figma:inspect` | Use the target rules above. |
| Understand a FigJam board or Slides deck | Read-only `figma:run`, `figma:capture` | Use the target rules above; metadata is Design-only. |
| Read implementation context | `figma:design-context`, `figma:motion-context` | Use the target rules above. |
| Read design systems | `figma:variables`, `figma:design-system`, `figma:libraries` | Use the target rules above. |
| Execute Plugin API | `figma:run` | Use the target rules above and a workspace script/source. |
| Move assets | `figma:assets:apply`, `figma:assets:download` | Use the target rules above. |
| Verify visually | `figma:capture` | Use the target rules above; inspect the saved PNG with `view_image`. |
| Connect code components | `figma:code-connect:inspect`, `figma:code-connect:plan`, `figma:code-connect:apply`, `figma:code-connect:verify` | Use the target rules above and one Design file. |
| Use an official capability through its live schema | `figma:upstream:help`, `figma:upstream:list`, `figma:upstream:read`, `figma:upstream:call` | `list`/`read` are targetless; otherwise use the selected live schema and target rules above. |

## Implement And Verify

1. Use the target rules above. Use `figma:metadata` only for broad Design-file discovery; use a read-only `figma:run` script for FigJam or Slides structure, then use targeted `figma:inspect` and applicable context commands. Resolve the current workspace root before running a command and keep any script or manifest there. For large trees, page `figma:inspect` with the cursor returned by the previous page, preserving its target and selection options; pages are live reads rather than a snapshot.
2. Create `.figma.ts` files in the current workspace and invoke `figma:run` according to its leaf help. Keep the script path absolute inside the workspace and provide the workspace output directory whenever the command exposes an output option.
...
1. Use the inspect leaf to discover components available for mapping.
2. Use the plan leaf with an explicit manifest to write an immutable plan and obtain its `planDigest`.
3. Use the apply leaf as the only write command, supplying the plan and its confirmation digest as defined by leaf help. Missing or mismatched confirmation, an invalid target/artifact, stale snapshots, and unapproved conflicts return `not_started` before dispatch; otherwise it bulk-writes and verifies.
4. Use the verify leaf to safely report `matched`, `missing`, `mismatch`, or `unavailable`.

Use `replace` only intentionally. The CLI does not scan or validate source files. Keep the plan and digest unchanged. After `outcome_unknown`, run `verify` before any retry. Use the generic `figma:upstream:*` path for uncovered capabilities.
...
- A complete invocation result is published once through `outputFiles.resultFile` when inline output is omitted. The receipt has `kind: "figma-cli-result"`, `schemaVersion: 1`, `tool`, `invocation`, and `result`; direct and typed protocol calls additionally expose sanitized `upstream`. Protocol `_meta` and tool-definition annotations are omitted, while an ordinary business `_meta` inside `structuredContent` is preserved. Over-budget direct responses return a bounded resource-limit diagnostic without persisting the payload.
