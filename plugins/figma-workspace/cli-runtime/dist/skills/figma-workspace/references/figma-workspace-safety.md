...
- Direct upstream calls use the selected live schema and do not receive `.figma.ts` preflight, private PluginData checks, or typed argument validation. Within the response budget, their sanitized business data is available through the single result receipt and omits protocol `_meta` and tool annotations while preserving business data, including a business `_meta` nested in `structuredContent`. An over-budget response does not persist its payload and returns a bounded resource-limit diagnostic with the existing `executionOutcome`.
