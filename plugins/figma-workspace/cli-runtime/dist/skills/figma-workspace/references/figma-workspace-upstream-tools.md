...
- These commands preserve selected-file context, local staging, diagnostics, and typed result conventions. If a wrapper returns `guidanceRef`, send its query to `figma:guidance`.
...
- Treat upstream output as transport-owned data; do not assume wrapper-normalized fields. Keep writes scoped to the selected file and node, then use capture and `view_image` for visible changes.
