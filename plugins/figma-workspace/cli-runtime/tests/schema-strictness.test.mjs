...
test("stateless invocation arguments are strict and reject removed state fields", async () => {
  ...
  try {
    for (const input of [
      { file: "Key", surface: "design", source: "return {};", sessionId: "legacy" },
      { file: "Key", surface: "design", source: "return {};", workspaceDir: "C:/legacy" },
      { file: "Key", surface: "design", source: "return {};", inputFile: "legacy.figma.ts" },
      { file: "Key", surface: "design", source: "return {};", unexpected: true },
...
test("run requires exactly one strict TypeScript source and explicit file", async () => {
  ...
  try {
    ...
    await assert.rejects(current.run({ file: "Key", surface: "design" }), /Exactly one/iu);
    await assert.rejects(current.run({ file: "Key", surface: "design", source: "return {};", scriptPath: "x.figma.ts" }), /Exactly one/iu);
    await assert.rejects(current.run({ file: "Key", source: "return {};" }), /surface/iu);
  } finally { await current.close(); }
});
...
test("stable node inputs reject dynamic selectors and incomplete context", async () => {
  ...
  try {
    for (const target of ["$selection", "$currentPage", "$legacy"]) {
      await assert.rejects(current.inspect({ file: "https://www.figma.com/design/Key/UI", target }), /stable|dynamic|does not accept/iu);
    }
    await assert.rejects(current.captureNode({ target: "1:2" }), /requires "file"/iu);
    await assert.rejects(current.getDesignContext({ file: "Key", surface: "design", target: { fileKey: "Key" } }), /exact|nodeId/iu);
  } finally { await current.close(); }
});
...
test("boolean, numeric, and nested arrays do not coerce", async () => {
  const current = client();
  try {
    await assert.rejects(current.captureNode({ file: "Key", surface: "design", target: "1:2", contentsOnly: "false" }), /contentsOnly.*boolean/iu);
    await assert.rejects(current.inspect({ file: "Key", surface: "design", target: "1:2", depth: "2" }), /depth.*integer/iu);
    await assert.rejects(current.searchDesignSystem({ file: "Key", surface: "design", query: "button", includeLibraryKeys: [7] }), /includeLibraryKeys.*string array/iu);
