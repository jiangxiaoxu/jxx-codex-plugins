...
- The `figma:upstream:call` JSON command exposes `--input`, `--state-file`, and `--max-inline-bytes`; its public help contains the complete input schema and rejects unknown fields. Pass `--input -` to read JSON from stdin, subject to the 256 KiB public JSON-input limit.
...
- Before repeating an upstream mutation after a timeout, cancellation, or transport loss, inspect and reconcile its intended Figma effect. Do not assume that a missing response proves the remote operation did not run.
