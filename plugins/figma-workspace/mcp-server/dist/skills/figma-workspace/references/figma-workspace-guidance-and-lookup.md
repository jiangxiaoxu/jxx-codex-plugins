...
- Use `figma:docs:search -- <query> --scope <active|conditional|examples|all> --state-file <absolute-path>` for compact BM25-ranked project documentation and canonical workflow snippets, including runtime-owned explanations for `guidanceRef`, wrapper profiles, helper profiles, and workflow graph routing. The default `active` scope uses 46 canonical records; select the 20 conditional records explicitly, select the 9 plugin-owned TypeScript-in-Markdown templates explicitly, or use `all` to include examples and the 12 router records.
- Use `figma:api:search -- <symbol> --state-file <absolute-path>` for exact Figma Plugin API symbols from the generated bundled `@figma/plugin-typings` symbol index. Both search commands accept `--limit` and `--snippet-lines`.
...
- `figma:guidance` uses the `active` canonical scope and exposes compact helper categories, wrapper profiles, and workflow graph nodes; use it instead of reading internal corpus files for sequencing.
- Runtime workflow lookup reads only the bundled plugin-owned `canonical-corpus/`, a self-contained 87-record corpus. Upstream source text and the development snapshot are not packaged or runtime inputs. The API search index is generated separately from bundled `@figma/plugin-typings`.
...
- Typed failures still emit Markdown and normally exit 1; an unhealthy doctor observation is the exit-0 exception. `figma:doctor` reports canonical corpus, generated API index, project-doc, and TypeScript asset health. Usage exits 2, typed interrupts exit 130, and thrown failures are text on stderr.
