...
import { createFigmaWorkspaceUpstreamStdioServer } from "../dist/mcp/index.js";

const packageRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..");

test("upstream stdio MCP server proxies tools and resources to the Figma client", async () => {
  const calls = [];
  let connected = false;
  const fakeClient = {
    async connect() {
  ...
  const { server } = createFigmaWorkspaceUpstreamStdioServer({ client: fakeClient });
...
test("upstream stdio CLI exits cleanly when stdin ends", async () => {
...
function runCliWithClosedStdin() {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, ["dist/upstream/upstream-stdio-bin.js"], {
      cwd: packageRoot,
