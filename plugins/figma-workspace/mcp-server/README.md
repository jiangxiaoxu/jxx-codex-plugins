...
The 88-record raw upstream snapshot is pinned to a GitHub commit with provenance and SHA-256 integrity data, and is not a default retrieval surface. The derived active index supplies 46 default `active` records. Its 20 `conditional` records require `docs:search --scope conditional`; its 12 `router` records are included only by `--scope all`; and its 9 `examples` records are non-executable and included only by `--scope examples` or `--scope all`. The one API record is available only through `figma:api:search`. `figma:doctor` reports raw snapshot, active-index, and pending/retired status. Regeneration and derived publication are performed only by the GitHub-only `npm run update:upstream-corpus -- --ref <git-ref>` updater.
