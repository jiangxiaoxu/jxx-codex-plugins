...
Fatal TypeScript preflight diagnostics report `executionOutcome: "not_started"`: repair them before rerunning. For `failed_atomic`, retain the direct host/script diagnostics, repair the script, and retry safely: Figma confirmed the file is unchanged. For `outcome_unknown`, follow `retryGuidance`, inspect returned IDs first, then narrowly read back deterministic content names in the intended slide before deciding what is missing. Shared PluginData is only an optional, host-verified supplement. If available fonts or existing branding do not support the requested direction, keep the deck consistent and report the restriction rather than substituting unreviewed typography. A still capture confirms layout and legibility, not presenter timing or animations.
