#!/usr/bin/env node
/**
 * Microsoft Store MSIX/APPX downloader.
 *
 * Resolves direct Microsoft Store package URLs through the Windows Update
 * SOAP API at fe3.delivery.mp.microsoft.com.
 *
 * Usage:
...
// The Windows Update endpoint can omit an intermediate CA. Add the Microsoft
// root and update CA certificates so Node can build the trust chain.
...
// Replace the default HTTPS CA set with the augmented certificate list.
...
// Anonymous device token compatible with the Windows Update Store API.
const DEVICE_TOKEN =
...
// Logging is enabled only when the file is invoked as a CLI.
...
async function getCookie() {
  ...
  if (res.status !== 200) {
    throw new Error(`GetCookie failed: HTTP ${res.status}`);
  }

  const parsed = parseXml(res.body);

  // Search deeply for EncryptedData.
  const cookie = deepFind(parsed, "EncryptedData");
  if (!cookie) {
    throw new Error("Cannot extract EncryptedData from response.");
  }

  log(`    Cookie length: ${cookie.length}`);
  return cookie;
}

async function getAppInfo(productId, market = "US", locale = "en-US") {
  log(`  [2/4] Query app info: ${productId}...`);
  ...
  if (res.status !== 200) {
    throw new Error(`GetAppInfo failed: HTTP ${res.status}`);
  }
...
async function getFileList(cookie, categoryId, ring) {
  log(`  [3/4] Get file list (ring=${ring})...`);
  ...
  if (res.status !== 200) {
    throw new Error(`SyncUpdates failed: HTTP ${res.status}`);
  }

  // XML fragments in the response are HTML-encoded.
  let body = res.body.replace(/&lt;/g, "<").replace(/&gt;/g, ">");
  const parsed = parseXml(body);

  // Extract File nodes into {name: {extension, size, digest}}.
  ...
  // Extract UpdateIdentity and PackageMoniker.
  const updates = [];
  ...
  log(`    Found ${updates.length} package(s)`);
  return updates;
}
...
async function getDownloadUrl(updateID, revisionNumber, ring, digest) {
  ...
  // If digest matching fails, return the first URL.
  for (const loc of locations) {
    const url = deepFind(loc, "Url");
        ...
        .get(reqUrl, (res) => {
          ...
          res.on("data", (chunk) => {
            ...
            if (total > 0) {
              ...
              process.stdout.write(`\r    Downloading: ${pct}% (${formatSize(downloaded)} / ${formatSize(total)})`);
            }
          });
...
async function main() {
  ...
  if (args.length === 0 || args.includes("--help") || args.includes("-h")) {
    console.log(`
Usage: node fetch-msstore.js <productId> [options]

Options:
  --ring <ring>       Channel (Retail/RP/WIS/WIF). Defaults to Retail.
  --market <code>     Market code. Defaults to US.
  --download          Download resolved packages.
  --filter <keyword>  Filter package names, for example x64 or arm64.
  --out <dir>         Download output directory. Defaults to ./downloads.
...
`);
  ...
  // Normalize productId when a full URL was passed.
  ...
  if (!appInfo.categoryId) {
    console.error("\nCannot get CategoryID. This app might not be packaged as MSIX/APPX.");
    process.exit(1);
  }
  ...
  // Step 4: Get URLs
  console.log("  [4/4] Get download links...");
  const results = [];
  ...
  // Print results.
  console.log(`\n${"-".repeat(80)}`);
  console.log(`${appInfo.name} - ${filtered.length} package(s)\n`);

  for (const pkg of filtered) {
    const size = formatSize(pkg.size);
    const hasUrl = pkg.url ? "[ok]" : "[missing]";
    console.log(`  ${hasUrl} ${pkg.name}`);
    console.log(`     Size: ${size}`);
    if (pkg.url) {
      console.log(`     URL: ${pkg.url.slice(0, 120)}...`);
    }
    console.log();
  }
  ...
  if (doDownload) {
    ...
    if (toDownload.length === 0) {
      console.log("No downloadable packages.");
      return;
    }
  ...
  // Return JSON for callers that import this module.
  return filtered;
}

async function getLatestCodexMsix() {
  const cookie = await getCookie();
  const appInfo = await getAppInfo("9plm9xgg6vks", "US");
  if (!appInfo.categoryId) {
    throw new Error("Cannot get CategoryID for Codex.");
  }

  const packages = await getFileList(cookie, appInfo.categoryId, "Retail");
  const pkg =
    packages.find((item) => item.name.toLowerCase().includes("_x64__")) ||
    packages[0];
  if (!pkg) {
    throw new Error("No Codex MSIX package found.");
  }
...
async function downloadLatestMsstorePackage(options = {}) {
  const productId = options.productId || "9plm9xgg6vks";
  const ring = options.ring || "Retail";
  const market = options.market || "US";
  const filter = (options.filter || "x64").toLowerCase();
  const outDir = path.resolve(options.outDir || path.join(process.cwd(), "downloads"));

  const cookie = await getCookie();
  const appInfo = await getAppInfo(productId, market);
  if (!appInfo.categoryId) {
    throw new Error(`Cannot get CategoryID for product ${productId}.`);
  }

  const packages = await getFileList(cookie, appInfo.categoryId, ring);
  const candidates = packages.filter((item) => item.name.toLowerCase().includes(filter));
  const pkg = candidates.find((item) => item.name.toLowerCase().includes("_x64__")) || candidates[0];
  if (!pkg) {
    throw new Error(`No package found for filter ${filter}.`);
  }

  const url = await getDownloadUrl(pkg.updateID, pkg.revisionNumber, ring, pkg.digest);
  if (!url) {
    throw new Error(`No download URL found for ${pkg.name}.`);
  }
  ...
  return {
    ...
    name: pkg.name,
    url,
    size: Number(pkg.size) || null,
    digest: pkg.digest || null,
  };
}

// Module API for package-runtime.js.
module.exports = {
  getCookie,
  getAppInfo,
  getFileList,
